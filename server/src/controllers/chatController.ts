  ...
  try {
    ...
    if (!isGroup) {
      ...
      const existingChat = await Chat.findOne({
        isGroup: false,
        participants: { $all: [currentUserId, new Types.ObjectId(recipientId)] },
      ...
      const newChat = await Chat.create({
        participants: [currentUserId, new Types.ObjectId(recipientId)],
        isGroup: false,
      });

      const populatedChat = await Chat.findById(newChat._id).populate(
        'participants',
        'name email avatar bio status lastSeen'
      );
    ...
    const uniqueParticipants = Array.from(
      new Set([...participants.map((id) => id.toString()), currentUserId.toString()])
...
export const updateChat = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (groupImage !== undefined) updates.groupImage = groupImage;
  ...
  try {
    ...
    if (isAdmin) {
      ...
      chat.participants = chat.participants.filter(
        (partId) => partId.toString() !== currentUserId.toString()
      );
      await chat.save();
